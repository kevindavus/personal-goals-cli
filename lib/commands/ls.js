    ...
    if (stats.isDirectory()) {
      ...
      if (typeof opts.date === "string") {
        ...
        if (type === "weekly" && moment(opts.date, "MMMDDYYYYHHmm").diff(moment(), "day") < -6) {
          ...
          fs.moveSync(path.join(dir, item), path.join(accomplishments, type, moment().month(moment().get("m") - 1).format("MMMM-YYYY"), item));
        } else if (type === "yearly" && moment(opts.date, "MMMDDYYYYHHmm").get("year") < moment().get("year")) {
          fs.moveSync(path.join(dir, item), path.join(accomplishments, type, moment().year(moment().get("y") - 1).format("YYYY"), item));
        } else {
